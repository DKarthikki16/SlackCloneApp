      ...
      <View style={styles.profileContainer}>
        <TouchableOpacity onPress={handleImagePick}>
          ...
          <View style={[styles.profileStatusIndicatorContainer]}>
            <View style={[styles.statusIndicator, { backgroundColor: isUserActive ? (theme?.online || '#4A99E9') : (theme?.away || '#A3A3A3') }]} />
          </View>
        </TouchableOpacity>

        <Text style={[styles.name, { color: theme.text }]}>{name || 'Karthik_MD student'}</Text>
        {userStatus ? (
          <Text style={[styles.status, { color: theme.secondaryText }]}>{userStatus}</Text>
        ) : null}
        ...
        <View style={styles.contactContainer}>
          ...
          <View style={[styles.contactBox, { backgroundColor: theme.card }]}>
            <Text style={[styles.email, { color: theme.text }]}>{email || 'karthikd062004@gmail.com'}</Text>
            <Text style={[styles.emailType, { color: theme.secondaryText }]}>Work</Text>
          </View>
        </View>
      </View>
    </ScrollView>
